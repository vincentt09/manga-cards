  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-red-950/10 pb-24 md:pb-8 md:pt-14">
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-3 py-5 sm:px-4 sm:py-7">
        ...
        <section><div className="mb-3 flex flex-col items-start gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-display text-xl font-bold">Boss du multivers</h2><p className="text-xs text-muted-foreground">La première victoire donne 100 % des récompenses, les suivantes 20 %.</p></div><span className="text-xs font-bold text-primary">Palier max {state?.maxStage || 1}/{PVE_BOSSES.length}</span></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">{PVE_BOSSES.map((boss) => <BossCard key={boss.id} boss={boss} locked={boss.stage > (state?.maxStage || 1)} cleared={state?.clearedStages?.includes(boss.stage)} selected={selectedBoss.id === boss.id} onSelect={() => setSelectedBossId(boss.id)} />)}</div></section>
