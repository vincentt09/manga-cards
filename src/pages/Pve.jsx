...
import { PVE_BOSSES, PVE_MAX_ENERGY, getPveTeamPower } from "@/lib/pveData";
...
function BossCard({ boss, locked, cleared, selected, onSelect }) {
  return (
...
function BattleResult({ result, boss, onClose }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[90] grid place-items-center bg-black/85 p-4 backdrop-blur-md" onClick={onClose}>
      <motion.div initial={{ scale: 0.75, y: 30 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-md overflow-hidden rounded-3xl border border-white/15 bg-card shadow-2xl" onClick={(event) => event.stopPropagation()}>
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-red-950/10 pb-24 md:pb-8 md:pt-14">
      ...
      <main className="mx-auto max-w-6xl space-y-6 px-3 py-5 sm:px-4 sm:py-7">
        <section className="relative overflow-hidden rounded-3xl border border-red-500/20 bg-gradient-to-br from-red-950/40 via-card to-primary/10 p-5 sm:p-7">
          ...
          <div className="relative flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
            ...
            <div className="flex gap-2">
              <div className="rounded-2xl border border-border bg-background/50 px-4 py-3 text-center"><p className="text-[10px] text-muted-foreground">Victoires</p><p className="font-display text-xl font-bold text-emerald-400">{state?.wins || 0}</p></div>
        ...
        <section className="rounded-3xl border border-border bg-card/70 p-4 sm:p-5">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-display text-lg font-bold">Ton équipe</h2><p className="text-xs text-muted-foreground">Jusqu’à 5 cartes · retire une carte en cliquant dessus</p></div><div className="flex items-center gap-2"><span className="rounded-full bg-primary/10 px-3 py-1.5 text-xs font-bold text-primary">Puissance {teamPower.toLocaleString()}</span><Button size="sm" disabled={!deckChanged || selectedIds.length === 0 || saveDeck.isPending} onClick={() => saveDeck.mutate()}>{saveDeck.isPending ? "Sauvegarde…" : "Enregistrer"}</Button></div></div>
        ...
        <section><div className="mb-3 flex items-end justify-between"><div><h2 className="font-display text-xl font-bold">Boss du multivers</h2><p className="text-xs text-muted-foreground">La première victoire donne 100 % des récompenses, les suivantes 20 %.</p></div><span className="text-xs font-bold text-primary">Palier max {state?.maxStage || 1}/{PVE_BOSSES.length}</span></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">{PVE_BOSSES.map((boss) => <BossCard key={boss.id} boss={boss} locked={boss.stage > (state?.maxStage || 1)} cleared={state?.clearedStages?.includes(boss.stage)} selected={selectedBoss.id === boss.id} onSelect={() => setSelectedBossId(boss.id)} />)}</div></section>
        ...
        <section className={`relative overflow-hidden rounded-3xl border p-5 sm:p-6 ${selectedBoss.stage <= (state?.maxStage || 1) ? "border-primary/30 bg-primary/5" : "border-border bg-card/50 opacity-60"}`}>
          <div className="flex flex-col items-center justify-between gap-5 sm:flex-row"><div className="flex items-center gap-4"><div className={`grid h-16 w-16 place-items-center rounded-2xl bg-gradient-to-br ${selectedBoss.color} text-3xl shadow-xl`}>{selectedBoss.icon}</div><div><p className="text-[10px] font-bold uppercase tracking-widest text-primary">Combat sélectionné</p><h3 className="font-display text-xl font-bold">{selectedBoss.name}</h3><div className="mt-1 flex flex-wrap gap-3 text-xs text-muted-foreground"><span className="flex items-center gap-1"><Heart className="h-3 w-3 text-red-400" />{selectedBoss.hp.toLocaleString()} PV</span><span className="flex items-center gap-1"><Swords className="h-3 w-3" />{selectedBoss.attack}</span><span className="flex items-center gap-1"><Shield className="h-3 w-3" />{selectedBoss.defense}</span><span className="flex items-center gap-1"><Zap className="h-3 w-3" />{selectedBoss.speed}</span></div></div></div><div className="w-full sm:w-auto sm:text-right"><p className="mb-2 text-xs text-muted-foreground"><Coins className="mr-1 inline h-3 w-3 text-yellow-400" />{selectedBoss.rewardCoins.toLocaleString()} · <Sparkles className="mx-1 inline h-3 w-3 text-primary" />{selectedBoss.rewardXp} XP</p><Button size="lg" className="w-full bg-gradient-to-r from-red-600 to-orange-500 font-display font-bold sm:w-52" disabled={isLoading || fight.isPending || selectedIds.length === 0 || (state?.energy || 0) < 1 || selectedBoss.stage > (state?.maxStage || 1)} onClick={() => fight.mutate()}>{fight.isPending ? "Combat en cours…" : <><Swords className="mr-2 h-5 w-5" />Combattre · 1 ⚡</>}</Button></div></div>
        </section>
      </main>
