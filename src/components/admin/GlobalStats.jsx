...
import { 
  TrendingUp, Users, Package, Activity, Zap, Shield, Layers
} from "lucide-react";
import { RARITY_CONFIG } from "@/lib/gameData";

export default function GlobalStats({ profiles, cards, transactions, users, listings }) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-border bg-card p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-secondary/20">
              <span className="text-sm text-muted-foreground">Ventes actives</span>
              <span className="font-display text-lg font-bold text-blue-300">{activeListings} 📦</span>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl bg-secondary/20">
              <span className="text-sm text-muted-foreground">Ventes vendues</span>
              <span className="font-display text-lg font-bold text-green-300">{soldListings} ✅</span>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {Object.entries(txByType).map(([type, count]) => (
            <div key={type} className="p-4 rounded-xl bg-secondary/30 border border-border/50 text-center">
